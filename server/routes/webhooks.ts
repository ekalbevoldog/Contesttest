...
webhooksRouter.post('/webhook', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (webhookSecret) {
      ...
      console.warn('WARNING: Processing unverified webhook. Not recommended for production!');
    }
  } catch (err) {
      ...
      case 'checkout.session.completed': {
        ...
        if (checkoutSession.customer && checkoutSession.subscription) {
          if (checkoutSession.metadata?.userId) {
            ...
            console.error('Checkout session completed but no userId in metadata', checkoutSession.id);
          }
        }
        break;
      }
...
async function handleSubscriptionCancelled(subscription: Stripe.Subscription): Promise<void> {
  try {
    ...
    console.log(`Successfully processed subscription cancellation for user ${user.id}`);
  } catch (error) {
